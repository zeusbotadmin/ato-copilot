...
/// <summary>
/// MCP tool: compliance_link_poam_component — Link one or more components to a POA&amp;M item.
/// RBAC: ISSO, ISSM, AO, ComplianceOfficer
/// </summary>
public class LinkPoamComponentTool : BaseTool
{
    private readonly IServiceScopeFactory _scopeFactory;
    private static readonly JsonSerializerOptions JsonOpts = new() { WriteIndented = true };

    public LinkPoamComponentTool(IServiceScopeFactory scopeFactory, ILogger<LinkPoamComponentTool> logger) : base(logger)
    {
        _scopeFactory = scopeFactory;
    }

    public override string Name => "compliance_link_poam_component";

    public override string Description =>
        "Link one or more HW/SW inventory components to a POA&M item for asset-level risk tracking. " +
        "Validates same-system membership. RBAC: ISSO, ISSM, AO, ComplianceOfficer.";

    public override IReadOnlyDictionary<string, ToolParameter> Parameters => new Dictionary<string, ToolParameter>
    {
        ["poam_id"] = new() { Name = "poam_id", Description = "POA&M item ID (GUID)", Type = "string", Required = true },
        ["component_ids"] = new() { Name = "component_ids", Description = "Comma-separated component IDs to link", Type = "string", Required = true }
    };
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            await poamService.LinkComponentsAsync(poamId, componentIds, "mcp-user", cancellationToken);

            sw.Stop();
            return JsonSerializer.Serialize(new
            {
                status = "success",
                data = new { poam_id = poamId, linked_components = componentIds.Length },
                metadata = Meta(sw)
            }, JsonOpts);
        }
        catch (InvalidOperationException ex)
        {
            return Error("LINK_FAILED", ex.Message);
        }
        catch (Exception ex)
        {
            Logger.LogError(ex, "compliance_link_poam_component failed");
            return Error("LINK_FAILED", ex.Message);
        }
    }
