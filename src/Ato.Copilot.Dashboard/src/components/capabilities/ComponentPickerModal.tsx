      ...
      <div className="w-full max-w-lg rounded-xl bg-white shadow-2xl">
        ...
        <div className="px-6 py-3 border-b flex gap-2">
          <input
            ...
            className="flex-1 border rounded px-3 py-1.5 text-sm focus:ring-2 focus:ring-blue-300 focus:outline-none"
          />
          <select
            value={typeFilter}
            ...
            <div className="space-y-1">
              {components.map(c => {
                ...
                return (
                  <label
                    key={c.id}
                    className={`flex items-center gap-3 rounded-md px-3 py-2 cursor-pointer hover:bg-gray-50 ${isSelected ? 'bg-blue-50 border border-blue-200' : ''}`}
                  >
                    <input
                      type="checkbox"
                      checked={isSelected}
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-xs text-gray-500">
                        {c.componentType}
                        {c.subType && ` · ${c.subType}`}
                        {isLinked && <span className="ml-1 text-blue-600">(currently linked)</span>}
                      </div>
                    </div>
                  </label>
                );
